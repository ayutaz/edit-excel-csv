...
test.describe('ヘッダーナビゲーション', () => {
  test('「開く」ボタンでファイル選択ダイアログが開く', async ({ page }) => {
    ...
    await page.locator('[data-testid="app-header"]').getByRole('button', { name: '開く' }).click()
  ...
  test('エディタから別ファイルに切替できる', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="app-header"]')).toContainText('basic.csv', { timeout: 10_000 })
  })
  ...
  test('ウェルカム画面では「保存」ボタンがdisabledである', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="app-header"]').getByRole('button', { name: '保存' })
    ).toBeDisabled()
  })
})
